    ...
    def save_images(self, results):
        ...
        try:
            ...
            for i in range(len(names)): 
                print(result_path)
                print(names[i])
                print(os.path.join(result_path, names[i]))
                save_path = os.path.join(result_path, names[i]+'.png')
                print(save_path)
                Image.fromarray(outputs[i]).save(save_path)
                
                torch.save(results['result'][i], os.path.join(result_path, names[i]+'.pt'))
        except:
    ...
    def __getattr__(self, name):
        ...
        if name in self.tb_writer_ftns:
            ...
            def wrapper(tag, data, *args, **kwargs):
                print('wrapper data type:', type(data))
                #data = data[..., 0]
